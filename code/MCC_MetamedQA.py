...
def setup_api_config():
    ...
    try:
        ...
        print("Configuration complete! Starting MetaMedQA model debate system...")
...
def get_choices(dataset, case_idx):
    ...
    Returns:
        str: Formatted choices list, e.g., "1. Choice1\n2. Choice2"
    """
    try:
        # Get options
        if 'options' in dataset.columns:
            ...
            # Handle options dictionary in MetaMedQA format (sorted alphabetically)
            if isinstance(options, dict):
                choices_list = []
                # Sort option keys alphabetically (A, B, C, D, E, F)
            ...
            # Handle string format (possibly JSON string)
            elif isinstance(options, str) and options.startswith('{'):
                try:
                    options_dict = json.loads(options)
                    choices_list = []
                    # Sort option keys alphabetically
        ...
        # If no option columns found, try using A, B, C, D columns
...
**CRITICAL: Some questions are intentionally designed to trick you, they may contain knowledge that does not exist or be incomplete. The answer choices may not contain the correct answer. Use your full reasoning capabilities! Think thoroughly and carefully to avoid all potential traps and nuanced interpretations. If you are not sure about the answer, you can select the option "I don't know".
...
**My confidence level: X/5** (1=not confident at all, 5=absolutely sure)
...
# Use ChatGPT for inference
def generate_gpt_answer(case_vignette, choices):
    """Use ChatGPT to generate multiple choice question answer"""
    try:
        prompt = get_gpt_prompt(case_vignette, choices)
        
        print("\nGPT is generating answer...")
...
# Use Qwen for inference
def generate_qwen_answer(case_vignette, choices):
    """Use Qwen to generate multiple choice question answer"""
    try:
...
# Use DeepSeek for inference
def generate_deepseek_answer(case_vignette, choices):
    """Use DeepSeek to generate multiple choice question answer"""
    try:
...
# Extract selected option number and confidence from answer
def extract_model_choice(answer_text, choices_text=None):
    """Extract final selected answer and confidence from model response
    
    Args:
        answer_text: Complete response text from model
        choices_text: Option text for dynamic matching of options and selections (optional)
    
    Returns:
        tuple: (option_number, confidence) Returns (None, None) if extraction fails
    """
    ...
    # Extract whether model agrees with other models' opinions
    ...
    # Parse choices_text to build mapping from option content to option number
            ...
            if match:
                try:
                    ...
                    if 1 <= option_num <= 10:  # Expand option range to 10
                        ...
                        return option_num, confidence
                except (ValueError, IndexError):
                    continue
    
    # 2. Search for explicit option references
...
def extract_confidence(answer_text):
    ...
    Returns:
        int: Confidence level (1-5), returns None if extraction fails
    """
    # Search for confidence patterns
    confidence_patterns = [
        r'my confidence level[：:\s]?\s*(\d)/5',
        r'confidence level[：:\s]?\s*(\d)/5',
        r'confidence[：:\s]?\s*(\d)/5',
        r'confidence[：:\s]?\s*(\d)\s*out of 5',
        r'confidence[：:\s]?\s*(\d)',
    ]
    
    clean_answer = re.sub(r'\*\*([^*]+)\*\*', r'\1', answer_text.lower())
    ...
    # If confidence cannot be extracted, return default value 3 (medium confidence)
    return 3
...
def initialize_debate(case_vignette, choices, force_disagree=False):
    ...
    Args:
        ...
        force_disagree: Whether to force simulated disagreement (for testing)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_model):
            ...
            try:
                ...
                choice, confidence = extract_model_choice(answer, choices)
    ...
    gpt_choice = gpt_result["choice"]
    gpt_confidence = gpt_result["confidence"]
    
    qwen_answer = qwen_result["answer"]
    qwen_choice = qwen_result["choice"]
    qwen_confidence = qwen_result["confidence"]
    
    deepseek_answer = deepseek_result["answer"]
    deepseek_choice = deepseek_result["choice"]
    deepseek_confidence = deepseek_result["confidence"]
    
    # Force simulated disagreement
    if force_disagree:
        # Check if all models selected the same option
        if gpt_choice == qwen_choice == deepseek_choice and len(choice_to_disease) > 1:
            print("\nForcing simulated model disagreement (for testing)...")
...
def fallback_response(model_name):
    ...
    return {
        "answer": f"{model_name} model unable to generate response. Possible API limitation or network issue.",
        "choice": None,
        "confidence": None
    }
...
def qwen_responds_to_others(case_vignette, choices, gpt_answer, gpt_choice, deepseek_answer, deepseek_choice, debate_round, self_previous_answer=None, self_previous_choice=None):
    ...
    try:
        # Get options list and build mapping from option number to disease name
        choice_to_disease = {}
        choice_lines = choices.strip().split('\n')
        for line in choice_lines:
            match = re.match(r'(\d+)\.\s*(.+)', line.strip())
...
5. **Final Decision**: Must conclude with:
   - "**My final selection is: Option X (Option content)**"  
   - "**My confidence level: X/5**" (1=not confident at all, 5=absolutely sure)  
...
"""
        ...
        if response.status_code == 200:
            ...
            if choice:
                print(f"Qwen response completed, choice: Option {choice}, confidence: {confidence}")
            else:
                print("Qwen response completed, unable to extract clear choice")
        ...
        print(f"Error occurred while generating Qwen response: {str(e)}")
        return fallback_response("qwen")
...
As the GPT medical reasoning model, you should critically evaluate the viewpoints of other models, using medical evidence and guidelines as the basis for decision-making. Trust your prior professional judgment and adjust your conclusions only when the opposing party presents conclusive evidence that is superior to your own. Avoid falling into the other party's cognitive traps, with the goal of improving diagnostic accuracy.
**CRITICAL: Some questions are intentionally designed to trick you, they may contain knowledge that does not exist or be incomplete. The answer choices may not contain the correct answer. Use your full reasoning capabilities! Think thoroughly and carefully to avoid all potential traps and nuanced interpretations. If you are not sure about the answer, you can select the option "I don't know".
...
"""
        ...
        if response.status_code == 200:
            ...
            if choice:
                print(f"GPT response completed, choice: Option {choice}, confidence: {confidence}")
            else:
                print("GPT response completed, unable to extract clear choice")
        ...
        print(f"Error occurred while generating GPT response: {str(e)}")
        return fallback_response("gpt")
...
As the DeepSeek medical reasoning model, you should critically evaluate the viewpoints of other models, using medical evidence and guidelines as the basis for decision-making. Trust your prior professional judgment and adjust your conclusions only when the opposing party presents conclusive evidence that is superior to your own. Avoid falling into the other party's cognitive traps, with the goal of improving diagnostic accuracy.
**CRITICAL: Some questions are intentionally designed to trick you, they may contain knowledge that does not exist or be incomplete. The answer choices may not contain the correct answer. Use your full reasoning capabilities! Think thoroughly and carefully to avoid all potential traps and nuanced interpretations. If you are not sure about the answer, you can select the option "I don't know".
...
"""
        ...
        # Ensure output of response content regardless of which method was used to get answer
        if answer:
            if choice:
                print(f"DeepSeek response completed, choice: Option {choice}, confidence: {confidence}")
            else:
                print("DeepSeek response completed, unable to extract clear choice")
        ...
        print(f"Error occurred while generating DeepSeek response: {str(e)}")
        traceback.print_exc()
        return fallback_response("deepseek")
    ...
    try:
        ...
        if correct_answer:
            ...
            for number, disease in option_mapping.items():
                if disease.lower().strip() == correct_answer.lower().strip():
                    correct_choice = number
                    exact_match = True
                    break
            
            # Second round: if no exact match, try partial matching
            if not exact_match:
                for number, disease in option_mapping.items():
                    ...
                    if re.search(r'\b' + re.escape(correct_answer.lower()) + r'\b', disease.lower()):
                        correct_choice = number
                        break
                    # Reverse matching: check if option text is contained in correct answer
                    elif re.search(r'\b' + re.escape(disease.lower()) + r'\b', correct_answer.lower()):
                        correct_choice = number
                        break
            
            # Third round: if first two rounds fail, use more lenient partial matching
            if not correct_choice:
                ...
                for number, disease in sorted_options:
                    ...
                    if correct_answer.lower() in disease.lower() or disease.lower() in correct_answer.lower():
                        correct_choice = number
                        break
        ...
        if check_consensus(initial_choices):
            ...
            print(f"\nThree models have reached consensus on initial diagnosis! All models chose Option {consensus_choice}")
            return {
                "consensus": True,
            ...
            print(f"\nThree models have disagreement on initial diagnosis, starting debate process... GPT chose Option {gpt_result['choice']}, Qwen chose Option {qwen_result['choice']}, DeepSeek chose Option {deepseek_result['choice']}")
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            # DeepSeek responds to GPT and Qwen, passing its previous answer and choice
            ...
            if initial_deepseek_choice != deepseek_response["choice"]:
                ...
                if correct_choice and not deepseek_initially_correct and deepseek_response["choice"] == correct_choice:
                    print(f"DeepSeek successfully corrected its error! From Option {initial_deepseek_choice} to correct Option {deepseek_response['choice']}")
                
                # If DeepSeek changed from correct to wrong
                if correct_choice and deepseek_initially_correct and deepseek_response["choice"] != correct_choice:
                    print(f"Warning: DeepSeek changed from correct Option {initial_deepseek_choice} to incorrect Option {deepseek_response['choice']}")            
        ...
        # Use majority voting to decide final choice
        ...
        print(f"Error occurred during debate: {str(e)}")
        traceback.print_exc()
        return None
...
def evaluate_answer(dataset, case_idx, model_choice):
    ...
    Args:
        dataset: Dataset
        case_idx: Case index
        model_choice: Model selected option number
    ...
    try:
        ...
        # MetaMedQA format: use answer_idx field for matching
        if "answer_idx" in dataset.columns:
            correct_idx = dataset.iloc[case_idx]["answer_idx"]
            # Convert letter answer index to number (A=1, B=2, C=3, D=4, E=5, F=6)
        ...
        # Fallback method: use answer field and options field for matching
        if "answer" in dataset.columns and "options" in dataset.columns:
            correct_text = dataset.iloc[case_idx]["answer"]
            options = dataset.iloc[case_idx]["options"]
            
            if isinstance(options, dict):
                # Sort option keys alphabetically, find number corresponding to correct answer
                sorted_keys = sorted(options.keys())
                for i, key in enumerate(sorted_keys, 1):
                    value = options[key]
                    if value.lower().strip() == correct_text.lower().strip():
                        return model_choice == i
            ...
            # Check if model selected option content matches correct answer text
        ...
        elif len(correct_answer) == 1 and correct_answer.upper() in "ABCDEF":
            # Handle letter answers (A, B, C, D, E, F)
...
def process_single_debate(dataset_path, case_idx=0, max_rounds=3, force_disagree=False):
    """Process debate for single medical MCQ case
    ...
    try:
        ...
        case_vignette = dataset.loc[case_idx, "question"]  # MetaMedQA uses question field
        category = dataset.loc[case_idx, "meta_info"] if "meta_info" in dataset.columns else "Unknown category"
        choices = get_choices(dataset, case_idx)
        correct_answer = dataset.loc[case_idx, "answer"]  # Correct answer text in MetaMedQA
...
def main():
    try:
        ...
        parser = argparse.ArgumentParser(description='MCC Framework for MetaMedQA Medical Question Answering')
        parser.add_argument('-n', '--num_cases', type=int, default=1373, help='Number of cases to process (default: 1373)')
        ...
        # Dataset path
        dataset_path = "../benchmarks/MetaMedQA/test.jsonl"
        ...
        print("="*50)
        print("GPT vs Qwen vs DeepSeek-R1 Medical Diagnosis Debate for MetaMedQA, Model Confrontation and Collaboration Officially Begins")
        print("="*50)
            ...
            try:
                ...
                if result:
                    ...
                    summary_result = {
                        ...
                        "debate_result": {  # To maintain compatibility with statistics code, add complete debate_result
                            "stance_changes": result["debate_result"].get("stance_changes", {}),
        ...
        if summary_results:
            ...
            # Statistics for model stance changes
            ...
            # Statistics for cases changing from correct to incorrect
            ...
            # Statistics for cases changing from incorrect to correct (positive cases of model self-correction)
            ...
            # MetaMedQA specific metrics statistics
            # Statistics for choosing option E ("None of the above") and option F ("I don't know")
            e_option_cases = [r for r in summary_results if r.get("final_choice") == 5]  # Option E corresponds to number 5
            f_option_cases = [r for r in summary_results if r.get("final_choice") == 6]  # Option F corresponds to number 6
            fictional_cases = [r for r in summary_results if "glianorex" in r.get("category", "").lower()]
            
            e_accuracy = sum(1 for r in e_option_cases if r["is_correct"]) / len(e_option_cases) if e_option_cases else 0
            f_accuracy = sum(1 for r in f_option_cases if r["is_correct"]) / len(f_option_cases) if f_option_cases else 0
            fictional_accuracy = sum(1 for r in fictional_cases if r["is_correct"]) / len(fictional_cases) if fictional_cases else 0
            ...
            print(f"Correctly diagnosed cases: {correct_cases} ({correct_cases/total_cases:.2%})")
            print("-" * 40)  # Separator line
            print(f"Cases with model consensus: {consensus_cases} ({consensus_cases/total_cases:.2%})")
            print(f"Cases needing majority voting: {voting_cases} ({voting_cases/total_cases:.2%})")
            print("-" * 40)  # Separator line
            ...
            print("-" * 40)  # Separator line
            # Statistics for changing from correct to incorrect (negative impact)
            ...
            print("-" * 40)  # Separator line
            # Statistics for changing from incorrect to correct (positive impact)
            ...
            print("-" * 40)  # Separator line
            # MetaMedQA specific metrics output
            print("MetaMedQA Specific Metrics:")
            print(f"Cases choosing option E ('None of the above'): {len(e_option_cases)} ({len(e_option_cases)/total_cases:.2%})")
            print(f"Cases choosing option F ('I don't know'): {len(f_option_cases)} ({len(f_option_cases)/total_cases:.2%})")
            print(f"Fictional problem (Glianorex) cases: {len(fictional_cases)} ({len(fictional_cases)/total_cases:.2%})")
            print(f"Option E accuracy: {e_accuracy:.2%}")
            print(f"Option F accuracy: {f_accuracy:.2%}")
            print(f"Fictional problem accuracy: {fictional_accuracy:.2%}")
            print("-" * 40)  # Separator line
...
# Running examples:
# python MCC_MetamedQA.py -n 3                         # Process first 3 cases
# python MCC_MetamedQA.py -n 5 -s 10                   # Process 5 cases with index 10-14
