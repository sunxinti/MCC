...
def setup_api_config():
    ...
    try:
        ...
        print("Configuration complete! Starting MedQA model debate system...")
...
def get_choices(dataset, case_idx):
    ...
    """
    try:
        # Priority handling of MedQA dataset ending fields
        if 'ending0' in dataset.columns:
            choices_list = []
            ending_idx = 0
        ...
        # Handle RABBITS dataset choices field
...
def get_gpt_prompt(case_vignette, choices):
    """Build GPT multiple choice question prompt"""
    # Drug brand name identification task for RABBITS dataset
...
def generate_gpt_answer(case_vignette, choices):
    ...
    try:
        ...
        print("GPT is reasoning for answer...")
...
def get_qwen_prompt(case_vignette, choices):
    """Build Qwen multiple choice question prompt"""
    # Drug brand name identification task for RABBITS dataset
...
def generate_qwen_answer(case_vignette, choices):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'choices' in response_data and response_data['choices']:
                ...
                if 'message' in response_data['choices'][0]:
                    ...
                    # If content is empty, try to get content from reasoning_content
                    ...
                    if answer_length > 0:
                        print(f"First 100 characters of answercharacters: {answer[:100]}...")
                        print(f"Qwenanswer generation completed, time elapsed: {t_generate:.2f} seconds")
                        return answer
                    else:
        ...
        print(f"Qwen Error occurred while generating answer: {str(e)}")
        traceback.print_exc()
...
# Build DeepSeek multiple choice question prompt
...
def generate_deepseek_answer(case_vignette, choices):
    ...
    try:
        ...
        print(f"DeepSeek Error occurred while generating answer: {str(e)}")
        traceback.print_exc()
...
def extract_model_choice(answer_text, choices_text=None):
    ...
    # Strict matching of standard format "My final selection is: Option X (Option content)"
    ...
    # Search for strict matching patterns in the entire answer
    ...
    print("Warning: Unable to extract clear choice from answer, only strict matching format is supported")
    return None
...
def initialize_debate(case_vignette, choices, force_disagree=False):
    ...
    # Establish mapping from option numbers to disease names
    choice_to_disease = {}
...
def qwen_responds_to_others(case_vignette, choices, gpt_answer, gpt_choice, deepseek_answer, deepseek_choice, debate_round, self_previous_answer=None, self_previous_choice=None):
    ...
    try:
        # Get options list, Establish mapping from option numbers to disease names
        choice_to_disease = {}
        choice_lines = choices.strip().split('\n')
        for line in choice_lines:
            match = re.match(r'(\d+)\.\s*(.+)', line.strip())
...
def fallback_response(model_name):
    ...
    return {
        "answer": f"{model_name} model unable to generate response. Possible API limit or network issue.",
        "choice": None
    }
...
"""
        ...
        # Ensure response content is output regardless of which method is used
        if answer:
...
# Let models conduct debate and judge whether consensus is reached
def conduct_debate(case_vignette, choices, correct_answer, max_rounds=3, force_disagree=False):
    ...
    try:
        ...
        # Check if consensus has been reached initially
        ...
        # Track latest results from each model in each debate round
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            if initial_deepseek_choice != deepseek_response["choice"]:
                ...
                if correct_choice and not deepseek_initially_correct and deepseek_response["choice"] == correct_choice:
                    print(f"DeepSeek successfully corrected the error! Changed from option {initial_deepseek_choice} to correct option {deepseek_response['choice']}")
        ...
        if initial_deepseek_choice != deepseek_final_choice:
            ...
            if correct_choice and not deepseek_initially_correct and deepseek_final_choice == correct_choice:
                print(f"DeepSeek successfully corrected the error! Changed from option {initial_deepseek_choice} to correct option {deepseek_final_choice}")
...
def evaluate_answer(dataset, case_idx, model_choice):
    """Evaluate whether model choice is correct
    
    Args:
        dataset: Dataset
        case_idx: Case index
        model_choice: Option number selected by model
...
def process_single_debate(dataset_path, case_idx=0, max_rounds=3, force_disagree=False):
    ...
    try:
        ...
        # Get case data - adapt to MedQA, RABBITS and JSONL formats
        ...
        # Prioritize MedQA dataset's sent1 field, then use other fields
        if "sent1" in dataset.columns:
            ...
            case_vignette = dataset.loc[case_idx, "question"]  # JSONL uses question field
        else:
            case_vignette = "Question content not found"
            
        category = dataset.loc[case_idx, "meta_info"] if "meta_info" in dataset.columns else "Medical Multiple Choice"
        choices = get_choices(dataset, case_idx)
        
        # Get correct answer: MedQA uses label field, RABBITS uses output field
        if "label" in dataset.columns:
...
def main():
    try:
        ...
        # Dataset path - using MedQA dataset
        dataset_path = "../benchmarks/RABBITS/original/medqa_original.json"
        ...
        results_dir = "debate_results_original_MedQA"
            ...
            try:
                ...
                if result:
                    ...
                    summary_result = {
                        ...
                        "voting_needed": not result["debate_result"]["consensus"],  # If no consensus, majority vote is needed
                        "final_choice": result["debate_result"]["final_choice"],
                        "is_correct": result["is_correct"],
                        "stance_changes": result["debate_result"].get("stance_changes", {}),
                        "debate_result": {  # Add complete debate_result for compatibility with statistics code
                            "stance_changes": result["debate_result"].get("stance_changes", {}),
