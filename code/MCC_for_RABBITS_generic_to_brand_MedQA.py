...
def get_choices(dataset, case_idx):
    ...
    Returns:
        str: Formatted choices list, like "1. Choice1\n2. Choice2"
    """
    try:
        # Prioritize handling MedQA dataset's ending fields
        ...
        # Other cases (keep original logic)
...
def generate_gpt_answer(case_vignette, choices):
    """Use ChatGPT to generate multiple choice answers"""
    try:
        prompt = get_gpt_prompt(case_vignette, choices)
        
        print("GPT is reasoning answer...")
...
def generate_qwen_answer(case_vignette, choices):
    """Use Qwen to generate multiple choice answers"""
    try:
...
def generate_deepseek_answer(case_vignette, choices):
    """Use DeepSeek to generate multiple choice answers"""
    try:
...
def extract_model_choice(answer_text, choices_text=None):
    ...
    # Preprocessing: remove Markdown tags for more accurate matching
...
"""
        ...
        try:
            ...
            t_generate = time.time() - t_generate_start
            print(f"DeepSeek answer generation completed, time elapsed: {t_generate:.2f} seconds")
            # Extract DeepSeek's choice from generated answer
            choice = extract_model_choice(answer, choices)
            ...
            url = QWEN_API_URL  # Use configured URL instead of hardcoded
            headers = QWEN_HEADERS  # Use configured headers instead of hardcoded
...
def evaluate_answer(dataset, case_idx, model_choice):
    ...
    try:
        ...
        # Prioritize handling the label field of MedQA dataset
        if "label" in dataset.columns:
            correct_label = dataset.iloc[case_idx]["label"]
            # label field directly represents the correct ending index (starting from 0)
            # Model choice starts from 1, so need to add 1
            correct_answer_num = int(correct_label) + 1
            return model_choice == correct_answer_num
        
        # Handle the correct_choice field of RABBITS dataset
...
def process_single_debate(dataset_path, case_idx=0, max_rounds=3, force_disagree=False):
    ...
    try:
        ...
        # Prioritize using sent1 field from MedQA dataset, then use other fields
...
def main():
    try:
        ...
        log_dir = "logs_medqa_generic_to_brand"
        ...
        dataset_path = "../benchmarks/RABBITS/generic_to_brand/medqa_generic_to_brand.json"
        ...
        results_dir = "debate_results_medqa_generic_to_brand"
